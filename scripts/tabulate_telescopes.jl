...
telescope_ATL_n1 = config["observers"][1]
telescope_ATL_n2 = config["observers"][2]

config = JSON.parsefile(joinpath(@__DIR__, "configs/config_MTTP4.json"))
telescope_LA_n1 = config["observers"][2]
telescope_LA_n2 = config["observers"][4]
...
for (name,telescope) in zip(names, telescopes)
    ...
    _jds_night = TelescopeTasking.earliest_night(telescope["jd0_ref"], _observer_lla, eop_iau1980)
